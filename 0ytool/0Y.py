...
try:
    ...
    os.system('pip install pip-licenses');os.system('pip install licensing')
try:
    import paramiko
except:
...
{color.WHITE}            port     :{color.YELLOW} {port}
                ...
                print(f"{color.WHITE}[{color.BLUE}{date}{color.WHITE}] {color.WHITE}[{color.RED}Bad Login{color.WHITE}]  {color.YELLOW}{user}{color.WHITE}:{color.YELLOW}{password}")
    elif status == "0,1":
...
{color.WHITE}            port     :{color.YELLOW} {port}
                ...
                print(f"{color.WHITE}[{color.BLUE}{date}{color.WHITE}] {color.WHITE}[{color.RED}Bad Login{color.WHITE}]  {color.YELLOW}{username}{color.WHITE}:{color.YELLOW}{pas}")
    elif status == "1,1":
        for user in open(username,"r").read().splitlines():
            date=datetime.datetime.now().time()
            try:
...
{color.WHITE}            port     :{color.YELLOW} {port}
                ...
                print(f"{color.WHITE}[{color.BLUE}{date}{color.WHITE}] {color.WHITE}[{color.RED}Bad Login{color.WHITE}]  {color.YELLOW}{user}{color.WHITE}:{color.YELLOW}{password2}")
            num=num+1
...
{color.WHITE}            port     : {color.YELLOW}{port}
                ...
                print(f"{color.WHITE}[{color.BLUE}{date}{color.WHITE}] {color.WHITE}[{color.RED}Bad Login{color.WHITE}]  {color.YELLOW}{username2}{color.WHITE}:{color.YELLOW}{pas}")
                client.close()
        num=num+1
...
""")
    if port == None:
        top_ports=[21,22,23,25,53,80,110,111,135,139,143,443,445,993,995,1723,3306,3389,5900,8080,8443,10000,50000,32768,32769,32770,32771,49152,49153,49154,49155,49156,49157,49158,49159,49160,49161,49162,49163,49164,49165,49166,49167,49168,49169,49170,49171,49172,49173,49174,49175,49176,49177,49178,49179,49180,49181,49182,49183,49184,49185,49186,49187,49188,49189,49190,49191,49192,49193,49194,49195,49196,49197,49198,49199,49200,49201,49202,49203,49204,49205,49206,49207,49208,49209,49210,49211,49212,49213,49214,49215,49216,49217,49218,49219,49220,49221,49222,49223,49224]
        ...
        for porT in top_ports:
            ...
            if states["scan"][str(host)]["tcp"][porT]["state"] == "open":
                ...
                print(f"\r{color.WHITE}[{color.BLUE}{date}{color.WHITE}] {color.WHITE}[{color.GREEN}OPEN{color.WHITE}]  >>{color.YELLOW} TCP{color.WHITE} | {color.GREEN}{porT} {color.WHITE}/ {color.YELLOW} {name} | {product} | {version} | {extrainfo}")
            else:
                print(f"\r{color.WHITE}[{color.BLUE}{date}{color.WHITE}] {color.WHITE}[{color.RED}CLOSE{color.WHITE}] >>{color.RED} {porT} ",end="")
    else :
        scann_ports=nmap.PortScanner()
        states=scann_ports.scan(str(host),str(port))
        if states["scan"][str(host)]["tcp"][port]["state"] == "open":
...
{color.WHITE} version [0.1] {color.YELLOW}          ░▒▓████████▓▒░  ░▒▓█▓▒░   ░▒▓█▓▒░ 
   ...
   """)
            ...
            Error=Error+1   
            print(f"\r{color.WHITE}[{color.BLUE}{date}{color.WHITE}] [{color.RED}NOT TRUE {Error}{color.WHITE}] >> {host}:{passwd}{color.WHITE}",end="")
...
def web_paths(host,path):
    ...
    for dir in paths:
        ...
        try:
            ...
            if response == 200 :
                print(f"\r{color.WHITE}[{color.BLUE}{date}{color.WHITE}] {color.WHITE} >>> {color.GREEN}{web} {color.WHITE}| {color.GREEN}{response}{color.RESET}")
                true_path.append(web)
            elif response == 404 :
                print(f"\r{color.WHITE}[{color.BLUE}{date}{color.WHITE}] {color.WHITE} >>> {color.RED}{web} {color.WHITE}| {color.RED}{response}{color.RESET}")
            else :
                print(f"\r{color.WHITE}[{color.BLUE}{date}{color.WHITE}] {color.WHITE} >>> {color.YELLOW}{web} {color.WHITE}| {color.YELLOW}{response}{color.RESET}")
        except:
            break
    for i in true_path:
            date=datetime.datetime.now().time()
...
{color.WHITE} version [0.1] {color.YELLOW}          ░▒▓████████▓▒░  ░▒▓█▓▒░   ░▒▓█▓▒░ 
    ...
    """)
            print(f"\r{color.WHITE}[{color.BLUE}{date}{color.WHITE}] {color.WHITE} >>> {color.GREEN}{i} {color.WHITE}| {color.GREEN}200{color.RESET}")
...
def dos(host=None,thread=None,port=None):
    ...
    def send_packets():
        while True:
            try:
                date=datetime.datetime.now().time()
                print(f"{color.WHITE}[{color.BLUE}{date}{color.WHITE}]{color.YELLOW} Done Send new requests to {color.GREEN}{host} ")
...
""")
            ...
            print(f"{color.WHITE}[{color.BLUE}{date}{color.WHITE}]{color.RED} {md5_hex}")
...
if __name__ == "__main__":
    ...
    if str(args.type).upper() == "SSH":
        ...
        if args.ufile and args.pas :
            ftp_login(username=args.ufile,password=args.pas,ftp_server=args.host,port=args.port,status="1,0")
        elif args.pfile and args.user :
            ftp_login(username=args.user,password=args.pfile,ftp_server=args.host,port=args.port,status="0,1")
        elif args.ufile and args.pfile :
            ftp_login(username=args.ufile,password=args.pfile,ftp_server=args.host,port=args.port,status="1,1")
